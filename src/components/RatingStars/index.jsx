...
const RatingStars = ({ rating }) => {  // Prends la prop rating en paramètre (nb d'étoiles rouges à afficher)
  ...
  const renderStars = () => {   // Génère les étoiles à afficher en fonction du rating
    ...
    for (let i = 0; i < redStars; i++) {
      stars.push(<img key={i} className="stars" src={redstar} alt="étoile rouge" />); // Rajout des étoiles avec la méthode push au tableau stars
    }

    for (let i = 0; i < grayStars; i++) {
      stars.push(<img key={redStars + i} className="stars" src={graystar} alt="étoile grise" />);
    }
  ...
  return (
  <div className="rating">{renderStars()}</div> // Rendu du tableau stars dans une div
  );
};
